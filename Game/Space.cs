...
class Space : Node
{
    ...
    private static Trash[] trashList = new Trash[]{
        ...
        new Trash("frame", "metal", false),
        new Trash("solar_panel", "electronics", false),
        new Trash("banana_peel", "organic", false),
        new Trash("phone", "electronics", true),
        new Trash("aa_battery", "hazardous_waste", true),
        new Trash("car_battery", "hazardous_waste", true),
        new Trash("shirt", "textile", false),
        new Trash("plank", "wood", false),
        ...
        new Trash("oil_clump", "hazardous_waste", true),
        new Trash("toxic_drum", "hazardous_waste", true),
        new Trash("medical_waste", "hazardous_waste", true),
        ...
        for (int i = 0; i < mapSize; i++)
        {
            for (int h = 0; h < mapSize; h++)
            {
                ...
                foreach (var item in showTrash)
                {
                    ...
                    {
                        makemap = makemap.Remove(makemap.Length - 3);
                        if (item.Value.GetForbiddenMaterial()) makemap += " \u2620\uFE0E ";
