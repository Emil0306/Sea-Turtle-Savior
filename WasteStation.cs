...
class WasteStation 
{
...

	{
		...
		if (trash.GetMaterial() == containerSortType)
		{
			...
			Console.ResetColor();
			pollutionmeter.ChangePollution(-5); // Ved korrekt Sortering g�r Pollutionmeter % ned.
		...
		else
		{
			...
			Console.ResetColor();
            pollutionmeter.ChangePollution(+5); // Ved forkert  Sortering gør Pollutionmeter % op.
